...
func main() {
	...
	// this can be done by calling `getSquareChan` function multiple
	// times, where individual function call returns a channel which sends square
	// of numbers provided by `chanInputNums` channel
	...
	// and using `WaitGroup` and multiple goroutines to receive square number,
	// we can send square numbers to `merged` channel and close it
	...
	// step 5: print sum when above `for loop` is done executing which is
	// after `chanMergedSqr` channel closes
