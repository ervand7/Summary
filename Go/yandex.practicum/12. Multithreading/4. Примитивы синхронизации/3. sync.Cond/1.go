...
func (q *Queue) Push(t *Task) {
	...
	fmt.Println("Отправляем сигнал на получение горутины")
}
...
func (q *Queue) PopWait() *Task {
	...
	for len(q.arr) == 0 {
		/*
			Wait() вводит текущую горутину в режим ожидания до
			получения сигнала. При получении сигнала выполняется следующий
			после Wait код.
		*/
		q.cond.Wait()
	}
