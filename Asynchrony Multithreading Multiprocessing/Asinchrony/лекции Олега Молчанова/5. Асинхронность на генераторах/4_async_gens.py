...
Сокеты мы будем передавать в функцию select. А select будет нам возвращать те из
них, которые готовы.
...
чтобы поймать этот сокет и передать его в функцию select. А select нам сделает 
...
Словари dict_to_read и dict_to_write будут неким сырьем для tasks. Из этих словарей
мы будем извлекать генераторы и добавлять в список tasks

Обычно для такого дела (tasks) используют очереди (from collections import deque)
...
def event_loop():
    ...
    while any([tasks, dict_to_read, dict_to_write]):
        ...
        # мы должны обеспечить работой наш событийный цикл, поэтому давайте наполним список tasks генераторами
        while not tasks:
...
1.1 После инициализации переменных dict_to_read=to_write={} первым делом в программе 
...
1.3 Словарь dict_to_read становится таким:
key:
...
2.1 В первом (главном) окне терминала получаем сообщение
...
Важно понимать, что принцип Round Robin у нас реализуется именно в dict_to_read. 
Сейчас в dict_to_read будут меняться местами серверный и клиентский сокеты.
...
4.1 Теперь по принципу Round Robin, который у нас реализуется в dict_to_read,
в dict_to_read будут меняться местами 3 сокета: серверный, первого клиента и второго клиента.
И dict_to_read будет выглядеть так:
{
