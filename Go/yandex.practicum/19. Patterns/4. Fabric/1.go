...
func main() {
	...
	// Query to postgresql: SELECT something FROM list

	sqlite3 := NewConnector("sqlite3://...")
	sqlite3.Query("SELECT something FROM list")
	// Connect to sqlite3
	// Query to sqlite3: SELECT something FROM list
}
