...
// multiple inheritance like in python with mro.
contract FundMeTest is ZkSyncChainChecker, CodeConstants, StdCheats, Test {
    ...
    address public constant USER = address(1); // 0x0000000000000000000000000000000000000001

    // runs automatically before each individual test function is executed.
    ...
    function testFundUpdatesFundedDataStructure() public skipZkSync {
        // startPrank sets all subsequent calls' `msg.sender` to be the input address until `stopPrank` is called
        vm.startPrank(USER);
